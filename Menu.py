...
class NhanVien(QMainWindow, Ui_AgentWindow):
        def __init__(self):
                ...
                self.btnDelete.clicked.connect(self.xoaNV)
                self.btnAdd.clicked.connect(self.themNV)
                self.btnUpdate.clicked.connect(self.suaNV)
        def loaddata(self):
                query = db.cursor()
        ...
        def click_cell(self, row, column):
                ...
                self.txt_diachi.setText(self.WidgetDSNV.item(row,4).text())
        ...
        def themNV (self):
                maNV = self.txt_ma.text()
                tenNV = self.txt_ten.text()
                date = self.txt_ngaysinh.text()
                dNgaySinh = self.chuyen_doi_ngay(date)
                if self.rb_nam.isChecked() :
                        sGioiTinh = "Nam"
                else :
                        sGioiTinh = "Nữ"  
                sDiachi = self.txt_diachi.text()
                sSDT = self.txt_sdt.text()
                fHSL = self.txt_hsl.text()
                fLCB = self.txt_luong.text()
                dNgayVaoLam = self.chuyen_doi_ngay(self.txt_ngayvaolam.text())
                if not all([maNV, tenNV, dNgaySinh, sGioiTinh, sDiachi,sSDT,fHSL,fLCB,dNgayVaoLam]):
                        QMessageBox.warning(self, "Lỗi", "Vui lòng nhập đầy đủ thông tin!")
                        return
                try:
                        query = db.cursor()
                        query.callproc('InsertInto_tblnhanvien', (maNV, tenNV, dNgaySinh, sGioiTinh, sDiachi,sSDT,fHSL,fLCB,dNgayVaoLam))
                        db.commit()
        ...
        def suaNV (self):
                maNV = self.txt_ma.text()
                tenNV = self.txt_ten.text()
                date1 = self.txt_ngaysinh.text()
                dNgaySinh = self.chuyen_doi_ngay(date1)
                if self.rb_nam.isChecked() :
                        sGioiTinh = "Nam"
                else :
                        sGioiTinh = "Nữ"  
                ...
                try:
                        query = db.cursor()
                        query.callproc('Update_tblnhanvien', (maNV, tenNV, dNgaySinh, sGioiTinh, sDiachi,sSDT,fHSL,fLCB,dNgayVaoLam))
                        db.commit()
        ...
        def xoaNV(self):
                ...
                try:
                        query = db.cursor()
                        query.callproc('DeleteFrom_tblnhanvien',(maNV,))
                        db.commit()
                ...
                # Đóng kết nối với cơ sở dữ liệu
                        db.close()
        def chuyen_doi_ngay(self,ngay_thang):
                day, month , year = map(int, ngay_thang.split("/"))
                date = f"{year}-{month:02d}-{day:02d}"  # Sử dụng f-string để nối chuỗi và format cho month và day
                return date
